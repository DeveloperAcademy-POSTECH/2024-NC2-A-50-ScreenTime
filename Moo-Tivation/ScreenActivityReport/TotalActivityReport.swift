...
extension DeviceActivityReport.Context {
    ...
    static let listActivity = Self("List Activity")
}
...
// MARK: - Device Activity Report의 내용을 어떻게 구성할 지 설정
struct ListReport: DeviceActivityReportScene {
    //let totalInterval = UserSettingsManager.shared.loadHoursAndMinutes()
    // Define which context your scene will represent.
    /// 보여줄 리포트에 대한 컨텍스트를 정의해줍니다.
    let context: DeviceActivityReport.Context = .listActivity
    
    // Define the custom configuration and the resulting view for this report.
    /// 어떤 데이터를 사용해서 어떤 뷰를 보여줄 지 정의해줍니다. (SwiftUI View)
    let content: (ActivityReport) -> TotalListView
    ...
    func makeConfiguration(
        ...
        // Reformat the data into a configuration that can be used to create
        // the report's view.
        var totalActivityDuration: Double = 0.0 /// 총 스크린 타임 시간
        var list: [AppDeviceActivity] = [] /// 사용 앱 리스트
